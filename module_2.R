...
# Generate PCA
pca_data <- prcomp(t(processed_data), scale. = TRUE)
variance_explained <- round(100 * pca_data$sdev^2 / sum(pca_data$sdev^2), 2)

# Create color palette for groups
group_colors <- c("red", "blue", "green", "purple")[as.numeric(groups)]

# Create PCA plot
plot(pca_data$x[,1], pca_data$x[,2],
     col = group_colors,
     pch = 16,
     main = "PCA Plot - GSE285348 (After Normalization & Filtering)",
     xlab = paste("PC1 (", variance_explained[1], "%)", sep = ""),
     ylab = paste("PC2 (", variance_explained[2], "%)", sep = ""))
...
text(pca_data$x[,1], pca_data$x[,2], 
     labels = colnames(processed_data),
     pos = 3, cex = 0.6)
